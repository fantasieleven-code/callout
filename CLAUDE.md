...
**V0.3.5**: 18 MCP tools, 110 tests, npm published
...
npm run test         # vitest run (110 tests, 12 files)
...
```
src/
├── server.ts          # MCP Server入口 (~36行，bootstrap + 4 register calls)
├── util.ts            # 共享工具函数 (resolvePath, withPathHeader)
├── handlers/          # 工具注册模块 (4个文件)
│   ├── analysis.ts    # 7工具: review, challenge, guide, spot_check, test_translate, cleanup, validate
│   ├── todo.ts        # 4工具: todo_add, todo_update, todo_list, todo_summary
│   ├── recommend.ts   # 3工具: recommend, recommend_dismiss, recommend_reset
│   └── meta.ts        # 4工具: init, callout_help, save_review_findings, set_target_user
...
test/
├── server.test.ts     # 22 tests (集成测试: 18工具注册验证)
├── review.test.ts     # 10 tests
...
├── spot-check.test.ts # 6 tests
├── cleanup.test.ts    # 6 tests
