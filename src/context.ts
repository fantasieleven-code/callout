...
function detectTechStack(packageJson: Record<string, unknown> | null, files: string[]): string[] {
  const stack: string[] = [];
...
export async function collectContext(cwd: string): Promise<ProjectContext> {
  ...
  // Stats
  const codeFiles = files.filter(f => CODE_EXTENSIONS.has('.' + f.split('.').pop()));
  ...
  return {
    cwd,
    name: (packageJson?.name as string) || cwd.split('/').pop() || 'unknown',
