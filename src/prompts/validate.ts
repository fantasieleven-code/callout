...
${ctx.packageJson ? `- **Runtime**: ${(ctx.packageJson.dependencies as Record<string, string> | undefined) ? Object.keys(ctx.packageJson.dependencies as Record<string, string>).slice(0, 8).join(', ') : 'unknown'}` : ''}
